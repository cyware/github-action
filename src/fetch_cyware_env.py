"""
# Purpose:
Wait for the Cyware environment to be ready,
then put its data in the environment.
# Required environment variables:
- GITHUB_ENV
- GITHUB_REPOSITORY
- GITHUB_REF_NAME
- INPUT_API_TOKEN or CYWARE_API_TOKEN
- INPUT_TIMEOUT_MINUTES or CYWARE_TIMEOUT
- INPUT_APP_NAME or CYWARE_APP_NAME
"""
...
if not api_token:
    api_token = os.environ.get("CYWARE_API_TOKEN")
if not api_token:
    exit("No CYWARE_API_TOKEN provided, exiting.")

# Get the timeout
timeout_minutes = os.environ.get("INPUT_TIMEOUT_MINUTES")
if not timeout_minutes:
    timeout_minutes = os.environ.get("CYWARE_TIMEOUT")
if timeout_minutes:
    try:
        timeout_minutes = int(timeout_minutes)
    except Exception:
        exit(
            'ERROR: the CYWARE_TIMEOUT provided ("{}") is not an integer'.format(
                timeout_minutes
            )
        )
else:
...
if not app_name:
    app_name = os.environ.get("CYWARE_APP_NAME")
...
def fetch_cyware_environment():
    """Fetch the Cyware environment for this Github job"""
    ...
    if not len(response["data"]):
        exit("ERROR: no matching Cyware environment found")
...
def restart_environment(environment_id):
    """Restart the Cyware environment with the provided ID"""
...
def wait_for_environment():
    """Return the Cyware environment data once it's ready"""

    auto_restart = True
    was_restarted = False

    # Check the environment
    environment_id, environment_data = fetch_cyware_environment()
    ...
    while not environment_data["ready"]:
        ...
        if all([environment_data["retired"], auto_restart, not was_restarted]):
            ...
            was_restarted = True
            print("Restarted Cyware environment...")
        ...
        print("Waiting for Cyware environment...{}".format(wait_string))
        time.sleep(15)
...
def main():
    ...
    with open(bash_env_path, "a") as bash_env:
        bash_env.write(
            "\n".join(
                [
                    "CYWARE_BYPASS_TOKEN={}".format(environment_data["bypass_token"]),
                    "CYWARE_ENVIRONMENT_ID={}".format(environment_id),
                    "CYWARE_ENVIRONMENT_URL={}".format(environment_data["url"]),
                    "CYWARE_ENVIRONMENT_READY={}".format(environment_data["ready"]),
                    "CYWARE_ENVIRONMENT_RETIRED={}".format(
                        environment_data["retired"]
                    ),
                ]
                + ["CYWARE_ENVIRONMENT_COMMIT_HASH={}".format(commit_hash)]
                if commit_hash
                else []
            )
        )

    print("Cyware environment data written to {}!".format(bash_env_path))
